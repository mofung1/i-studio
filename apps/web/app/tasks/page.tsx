...
export default function TasksPage() {
  ...
  return <main className="content-page">
    <div className="content-page-head"><div><Link href="/" className="auth-back"><ArrowLeft size={16} />返回首页</Link><h1>任务中心</h1><p>查看所有生图任务和执行状态。</p></div><Link className="primary-action" href="/workbench?mode=general">新建任务</Link></div>
    {error && <p className="auth-notice">{error}</p>}
    <section className="task-list"><h2><Clock3 size={18} />最近任务</h2><div className="task-grid">{tasks.map((task) => <Link href={`/tasks/${task.id}`} key={task.id} className="task-item"><strong><ImageIcon size={16} />{task.input.productName ?? task.input.prompt?.slice(0, 24) ?? (task.input.mode === 'commerce' ? '电商图片' : '通用生图')}</strong><span>{statusLabels[task.status] ?? task.status} · {new Date(task.createdAt).toLocaleString()}</span></Link>)}{tasks.length === 0 && <p className="empty-state">暂无任务，先创建第一张图片。</p>}</div></section>
  </main>
}
